...
class StaticCallAnalyzer extends \Psalm\Internal\Analyzer\Statements\Expression\CallAnalyzer
{
    ...
    public static function analyze(
        ...
        if ($stmt->class instanceof PhpParser\Node\Name) {
            ...
            $lhs_type = $statements_analyzer->nodes->getNodeType($stmt->class) ?: Type::getMixed();
        }
            ...
            if ($stmt->name instanceof PhpParser\Node\Identifier && !$is_mock) {
                    ...
                    if ($codebase->methods->methodExists(
                        ...
                        if (isset($class_storage->pseudo_static_methods[$method_name_lc])) {
                            ...
                            if ($pseudo_method_storage->return_type) {
                                ...
                                if (!$stmt_type) {
                                    $statements_analyzer->nodes->setNodeType($stmt, $return_type_candidate);
                                } else {
                ...
                if ((!$stmt->class instanceof PhpParser\Node\Name
                    ...
                    if ($is_dynamic_this_method) {
                        ...
                        if ($fake_method_call_type = $statements_analyzer->nodes->getNodeType($fake_method_call_expr)) {
